...
def create_gpt_dataframe(gpt_dict):
    ...
    for comment_id, comment_dict in gpt_dict.items():
        row_data = {"comment number": comment_id,
                    "tone_1": comment_dict.get("tone_1"),
        ...
        for i in range(2,max_col_count):
            row_data["tone_{}".format(i)] = comment_dict.get("tone_{}".format(i),"")
...
def melt_wide_cols(gpt_df):
    ...
    '''
    mask_tone=["tone" in x for x in gpt_df.columns]
    ...
    gpt_tone = pd.melt(gpt_df,id_vars=id_vars, value_vars=gpt_df.columns[mask_tone], var_name="tone_number",value_name="tone")
    ...
    gpt2 = pd.concat([gpt_topic, gpt_segment.drop(labels=id_vars,axis=1), gpt_tone.drop(labels=id_vars,axis=1), gpt_intent.drop(labels=id_vars,axis=1)],axis=1)
